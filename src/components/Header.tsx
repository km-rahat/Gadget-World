...
export default function Header({
  ...
  const handleDropdownItemClick = (tab: 'profile' | 'orders') => {
    if (onTabChange) {
      onTabChange(tab);
    }
              ...
              <div className="relative shrink-0" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-2.5 w-48 bg-slate-950 border border-slate-850 rounded-xl shadow-2xl overflow-hidden py-1.5 z-60 animate-in fade-in slide-in-from-top-2">
                    <p className="px-3.5 py-2 text-[10px] uppercase font-mono tracking-widest text-slate-500 border-b border-slate-900 select-none">
                      Signed In As:<br/>
                      <span className="text-slate-300 lowercase font-sans select-text tracking-normal break-all block mt-0.5">{currentUser.email}</span>
                    </p>
                    <button
                    ...
                    <button
                      onClick={onSignOut}
                      className="w-full text-left px-3.5 py-2 text-xs text-red-400 hover:bg-red-950/20 transition-all flex items-center gap-2 border-t border-slate-900"
                    >
          ...
          <div className="pt-3 border-t border-slate-900 flex flex-col gap-2">
            {currentUser ? (
              <div className="space-y-2">
                <div className="text-[10px] text-slate-400 px-3 uppercase tracking-wider font-mono">
                  Signed in as: <strong className="text-white font-sans text-xs lowercase mt-0.5 block">{currentUser.email}</strong>
                </div>
                <div className="flex flex-col gap-1">
                  <button
                    onClick={() => handleDropdownItemClick('profile')}
                  ...
                  <button
                    ...
                    className="w-full text-left px-3 py-2 rounded-lg text-red-400 hover:bg-red-950/20 transition-all text-xs font-medium flex items-center gap-2"
                  >
                    <LogOut className="w-3.5 h-3.5" />
